node {
    ...
    stage("Unit tests") {
        try {
            sh "docker run -v \$(pwd)/m2:/root/.m2 -v \$(pwd):/app -w /app maven:3.5.3-jdk-8-alpine mvn clean test"
        } finally {
    ...
    stage("Integration tests") {
        try {
            sh "docker run -v \$(pwd)/m2:/root/.m2 -v \$(pwd):/app -w /app maven:3.5.3-jdk-8-alpine mvn clean test-compile failsafe:integration-test"
        } finally {
            junit 'target/failsafe-reports/**/*.xml'
        }
    }
    stage("Build artifact") {
        sh "docker run -v \$(pwd)/m2:/root/.m2 -v \$(pwd):/app -w /app maven:3.5.3-jdk-8-alpine mvn clean package"
    }
    stage("UAT") {
